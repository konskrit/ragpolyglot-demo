...
OCR: Tesseract for most languages; Kraken 7 for Ancient Greek (`grc`). User pause (`document.pause`) kills the Kraken/Python process group on Linux so OCR stops without waiting out the current CLI batch, then publishes `document.paused`. Unexpected CLI kills (signal / exit 137/143 / broken pipe) are **not** pauses: the worker retries from the last OCR checkpoint (streak of 3 consecutive no-progress aborts → `document.failed` reason `ocr_aborted`). No CUDA: Kraken retries on CPU. Kraken missing or still failing: Tesseract.

**Progress:** `document.progress` stages — `waiting_for_ocr` (unacked; quiet block on OCR ingest slot + 30s heartbeat; Ack only after the slot is taken; slot held across abort retries), `extracting` (Kraken/Tesseract; counts OCR’d pages per batch, not pdftoppm), `embedding`. **Concurrency:** `OCR_INGEST_PREFETCH` + `KRAKEN_GPU_CONCURRENT` (default = prefetch) are operator knobs; VRAM budget is split per concurrent job. Kraken batches render PDF pages in parallel (`PageWorkers`); CUDA runs use a sized semaphore (not a single global mutex).

**Retry / OCR language:** changing `ocrLang` on retry or via `/ocr-lang` sets `resetIngest` on `document.uploaded`. The worker wipes checkpoint + chunks and re-runs OCR from page 1. Same-language retry (and `ocr_aborted` auto-retry) still resumes from checkpoint.
