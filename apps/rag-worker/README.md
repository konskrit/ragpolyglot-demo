...
**Progress:** `document.progress` stages — `waiting_for_ocr` (unacked; blocked on OCR ingest slot with quiet wait + 30s heartbeat), `extracting` (Kraken/Tesseract; counts OCR’d pages per batch, not pdftoppm), `embedding`. **Concurrency:** `OCR_INGEST_PREFETCH` + `KRAKEN_GPU_CONCURRENT` (default = prefetch) are operator knobs; VRAM budget is split per concurrent job. Kraken batches render PDF pages in parallel (`PageWorkers`); CUDA runs use a sized semaphore (not a single global mutex).
